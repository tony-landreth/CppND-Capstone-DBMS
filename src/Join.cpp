...
Join::Join(std::unique_ptr<Projection> r, std::unique_ptr<Projection> s,
           std::vector<std::string> k, Schema rsch, Schema ssch)
    : r_(std::move(r)),
      s_(std::move(s)),
      keys(k),
      r_schema_(rsch),
...
Schema Join::getSchema() { return schema_; }
...
std::vector<std::string> Join::next() {
  ...
  // Use these variables to determine when the foreign keys map to matching
