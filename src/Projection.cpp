...
Projection::Projection(std::vector<std::string> column_names,
                       std::unique_ptr<PlanNode> sel, Schema sch)
    : keys(column_names), sel_(std::move(sel)), schema_(sch){};
...
std::vector<std::string> Projection::next() {
  ...
  // Now that you've projected into a lower-dimensional space, the file schema
  // is no longer useful
  return result;
}
...
void Projection::rewind() {
  ...
  if (sel_->keys.size() > 0) {
    std::unique_ptr<Selection> s =
        std::make_unique<Selection>(sel_->keys, std::move(fs), schema_);
    sel_ = std::move(s);
  } else {
    std::vector<std::string> where;
    std::unique_ptr<Selection> s =
        std::make_unique<Selection>(where, std::move(fs), schema_);
    sel_ = std::move(s);
  }
}
