...
class QueryPlanner {
 public:
  ...
  std::vector<std::vector<std::string> > binProjectionKeys(
      std::vector<std::string> prjKeys);
  std::vector<std::vector<std::string> > buildFrontEndColumnNames(
 ...
 private:
  ...
  std::map<std::string, std::vector<std::string> > mapQuery(
      TokenTree tt);  // generates an intermediate representation
